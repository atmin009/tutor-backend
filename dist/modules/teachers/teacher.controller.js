import { createTeacher, deleteTeacher, getTeacherById, listTeachers, updateTeacher, } from "./teacher.service.js";
import { error, success } from "../../utils/apiResponse.js";
export const listTeachersHandler = async (req, res, next) => {
    try {
...
export const getTeacherHandler = async (req, res, next) => {
...
export const createTeacherHandler = async (req, res, next) => {
...
export const updateTeacherHandler = async (req, res, next) => {
    const id = Number(req.params.id);
    if (Number.isNaN(id)) {
        return error(res, 400, "Invalid teacher ID");
    }
...
export const deleteTeacherHandler = async (req, res, next) => {
    const id = Number(req.params.id);
    if (Number.isNaN(id)) {
        return error(res, 400, "Invalid teacher ID");
    }
