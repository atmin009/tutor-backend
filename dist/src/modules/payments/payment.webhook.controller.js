...
import { verifyWebhookHash, checkTransactionStatusWithRetry, } from "./moneyspace.utils.js";
export const webhookHandler = async (req, res, next) => {
    try {
        ...
        if (hash && transaction_ID && amount !== undefined) {
            const isValidHash = verifyWebhookHash(hash, status, transaction_ID, amount, order_id);
            if (!isValidHash) {
        ...
        if (normalizedStatus === "paysuccess" ||
