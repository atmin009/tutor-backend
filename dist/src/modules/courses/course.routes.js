...
import { createCourseHandler, deleteCourseHandler, getCourseHandler, listCoursesHandler, updateCourseHandler, } from "./course.controller.js";
...
// Upload routes
courseRouter.post("/upload-cover", uploadImage.single("cover"), (req, res, next) => {
    try {
        if (!req.file) {
...
courseRouter.post("/upload-preview", uploadVideo.single("preview"), (req, res, next) => {
    try {
        if (!req.file) {
            return error(res, 400, "No file uploaded");
        }
