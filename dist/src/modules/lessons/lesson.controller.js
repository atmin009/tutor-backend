import { createLesson, deleteLesson, getLessonById, listLessonsBySection, updateLesson, } from "./lesson.service.js";
import { error, success } from "../../utils/apiResponse.js";
export const listLessonsHandler = async (req, res, next) => {
...
export const getLessonHandler = async (req, res, next) => {
...
export const createLessonHandler = async (req, res, next) => {
    const sectionId = Number(req.params.sectionId);
    ...
    const { title, contentType, contentUrl, contentText, duration, sortOrder, } = req.body ?? {};
...
export const updateLessonHandler = async (req, res, next) => {
    const id = Number(req.params.id);
    if (Number.isNaN(id)) {
        return error(res, 400, "Invalid lesson ID");
    }
    try {
        const { title, contentType, contentUrl, contentText, duration, sortOrder, sectionId, } = req.body ?? {};
        const updateData = {};
        if (title !== undefined) {
        ...
        if (contentText !== undefined)
            updateData.contentText = contentText || null;
...
export const deleteLessonHandler = async (req, res, next) => {
    const id = Number(req.params.id);
    if (Number.isNaN(id)) {
        return error(res, 400, "Invalid lesson ID");
    }
